...
const {data: lessonData, error: lessonError, refresh: refreshLesson, pending: lessonPending} = await useFetch('http://localhost:8000/api/lesson-by-id', {
      method: 'POST',
      body: {
        id: props.lessonId
      }
...
async function editLesson(){
  ...
  const {data: lessonEditData, error: lessonEditError, pending: lessonEditPending} = await useFetch('http://localhost:8000/api/edit-lesson', {
        method: 'POST',
        body: {
          id: props.lessonId,
          title: title.value,
...
function test(){
  console.warn('LESSON ID IS ' + props.lessonId)
}
