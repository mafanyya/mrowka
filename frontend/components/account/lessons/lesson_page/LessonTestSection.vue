        ...
        <template v-for = "question in testData.questions">
          <div :id ="'question-inner-' + question.number" :class="'question-inner-' + question.number">
            ...
            <div class="answers">
              <template v-for = "answer in testData.answers">
                <button  @click.prevent = "checkQuestion(question.number, answer.isTrue, answer.number)" v-if = "answer.questionId === question.id " :id = "'answer-' + answer.number + '-' + question.number" class="answer">
                  {{answer.title}}
                </button>
              </template>
            </div>
            <button @click.prevent = "refreshQuestion(question.number)" :id = "'next-btn' + question.number" class = "next-btn">Dalej</button>
          </div>
          <div :id = "'question-' + question.number + '-finish'" class="question-finish">
            ...
            <div class="clm-2">
              ...
              <button @click.prevent = "refreshLocalTest(question.number)" :id = "'refresh-btn-' + question.number" class = "refresh-test-btn">Spróbuj jeszcze raz</button>
            </div>
          </div>
        </template>
