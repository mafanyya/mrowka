            ...
            <div class="word">
              <div class="row-1">
                ...
                <div class="clm-2">
                  ...
                  <div class="delete">
                    <div @click.prevent="tryDelete(word.id, 1)" class="btn">
              ...
              <div :id="'word-' + word.id + '-remove-row'" class="row-2">
                <div class="inner">
                  <div class="clm-1">
                    <p :id="word.id + '-remove-message'"></p>
                  </div>
                  <div class="clm-2">
                    <button @click.prevent="tryDelete(word.id, 2)">Usuń</button>
                    <p @click.prevent="tryDelete(word.id, 3)">Wróć</p>
                  </div>
                </div>
              </div>
            </div>
...
} = await useFetch('http://localhost:8000/api/words')
if (wordsError.value) {
...
async function tryDelete(wordId, index) {
  let removeRow = document.getElementById('word-' + wordId + '-remove-row')
  let removeMessage = document.getElementById(wordId + '-remove-message')
  switch (index) {
    case 1:
      removeMessage.innerText = "Czy na pewno chcesz usunąć słowo?"
      removeRow.style.display = 'flex'
      break
    case 2:
      const {
        data: wordDeleteData,
        error: wordDeleteError,
        pending: wordDeletePending
      } = await useFetch('http://localhost:8000/api/remove-word', {
            method: 'POST',
            body: {
              id: wordId
            },
      ...
      if (wordDeleteData.value) {
        removeMessage.innerText = 'Słowo pomyślnie usunięte'
        setTimeout(removeRow.style.display = 'none', 10000)
        await refreshWords()
      }
      if (wordDeleteError.value) {
        removeMessage.innerText = 'Bład servera: słowo nie zostało usunięte'
        setTimeout(removeRow.style.display = 'none', 10000)
      }
      break
    case 3:
  ...
  if (!validateWordName(name.value)) {
    ...
    if (!validateWordTranslation(translation.value)) {
      ...
      const {
        data: wordAddData,
        error: wordAddError,
        pending: wordAddPending
      } = await useFetch('http://localhost:8000/api/add-word', {
            method: 'POST',
            body: {
              name: name.value,
...
async function findWordsBySection($sectionId) {
  const {
    data: wordsBySectionData,
    error: wordsBySectionError,
    pending: wordsBySectionsPending
  } = await useFetch('http://localhost:8000/api/words-by-section', {
        method: 'POST',
        body: {
          'sectionId': $sectionId
        },
...
async function findWordsByLesson($lessonId) {
  const {
    data: wordsByLessonData,
    error: wordsByLessonError,
    pending: wordsByLessonsPending
